...
var server = net.Server(function(socket) {
    ...
    socket.on('data', function(e) {
        ...
        if (!key) {
            ...
            // init others data
            let otherMsg = buildMsg(table.values, INIT_OTHER_TYPE);
            task({
                'to': [key],
                'except': [],
                'data': otherMsg
    ...
    function task(data, marker = {
        ...
        var out = extend(true, {}, marker),
            _data = [],
            _clients = clients.keys;

        out.PayloadData = JSON.stringify(data);
        out = encodeDataFrame(out);

        if (data.to.length > 0) {
            _clients = data.to;
        }
        logger.log(_clients.length);
        _clients && _clients.length && _clients.forEach(function(value, key, map) {
            if (data.to.findIndex(v => v = key) == -1) {
